...
**Ожидаемый результат**: форма отправлена, поле "Номер Карты" ограничено на ввод только 16ти значного числа, данное поле подсвечено красным и появляется подсказка "Неверный формат". В БД изменений не внесено.   
